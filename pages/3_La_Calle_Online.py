...
portal = next((p for p in PORTALES if p["nombre"] == "La Calle Online"), None)

with st.sidebar:
    st.title("🗞️ La Calle Online")
    st.markdown("---")
...
if portal is None or not credenciales_ok(portal):
    ...
    <div class="pendiente-box">
        ...
        <div style="color:white; font-size:28px; font-weight:800; margin:12px 0">
            La Calle Online — Pendiente de configuración
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
st.markdown(f"""
<div style="background:linear-gradient(135deg,#0f172a,#4a1942);
    ...
    with col_der:
        ...
        if posts_perf:
            top5 = sorted(posts_perf,
                          ...
                          reverse=True)[:5]
            items = [{"Publicación": ("🎬" if p["tipo"]=="reel" else
                                      "▶️" if p["tipo"]=="video" else
                                      "🖼️" if p["tipo"]=="carousel_album" else "📷") + f" {p['ts']}",
