...
portal = next((p for p in PORTALES if p["nombre"] == "El Americano"), None)

with st.sidebar:
    st.title("🌎 El Americano")
    st.markdown("---")
...
if portal is None or not credenciales_ok(portal):
    ...
    <div class="pendiente-box">
        ...
        <div style="color:white; font-size:28px; font-weight:800; margin:12px 0">
            El Americano — Pendiente de configuración
        </div>
        <div style="color:#64748b; font-size:15px">
            Una vez que la administradora envíe el token de acceso,<br>
            completá los datos en el archivo <code>.env</code> y el panel se activa automáticamente.
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("### 📋 Qué completar en `.env`")
    st.code("""META_PAGE_ID_AMERICANO=542904062233857
META_INSTAGRAM_ID_AMERICANO=17841472028591526
META_PAGE_ACCESS_TOKEN_AMERICANO=TOKEN_QUE_MANDE_EL_ADMIN""", language="bash")
    st.stop()
...
with st.spinner("Cargando estadísticas..."):
    ...
    try:
        ...
        datos_ig = {"info": {}, "impresiones": {"total_imp": 0, "total_reach": 0, "daily": {}, "posts_data": []},
                    "media": {"data": []}}
        error_ig = str(e)
...
st.markdown(f"""
<div style="background:linear-gradient(135deg,#0f172a,#1a3a4a);
...
with tab_ig:
    ...
    seg_ig2   = info_ig.get("followers_count", 0)
    ...
    c1.metric("👥 Seguidores",          f"{seg_ig2:,}")
