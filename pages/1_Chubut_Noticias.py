...
with tab_fb:
    ...
    for m in datos["fan_growth"].get("data", []):
        mname = m.get("name","")
        if "follow" in mname or "fan_adds" in mname:
...
# ═══════════════════════ INSTAGRAM ══════════════════════════════════
with tab_ig:
    # datos_ig ya cargado arriba junto con datos de FB
    ...
    c3.metric("🎯 Alcance único (30d)", f"{imp_ig['total_reach']:,}",
              help="Personas distintas que vieron el contenido (sin repeticiones)")
